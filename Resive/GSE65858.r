setwd("D:/workplace/mywork/revise/GEO/GSE65858/")
...
hsa16<-read.csv("enrichKEGG16.csv",header=T) #16条免疫衰老相关通路
pathway<-hsa16$ID%>%as.character()  #16条通路的ID向量

hsa_gene<-read.csv("HSA_KEGG.csv",header=T)   #KEGG通路和基因EntrzID对应列表
...
write.table(pathway_gene_result,"pathway_gene.txt")
###########通路基因的表达数据
...
sur.df2$sex<-sur.df2$sex %>%str_remove("gender: ")
...
sur.df2$stage<-sur.df2$stage%>%str_remove("uicc_stage: ")%>%as.factor()
sur.df2$smoking<-ifelse(sur.df2$smoking=="smoking: No",0,1)%>%as.numeric()
...
expression_clinical$stage=ifelse(expression_clinical$stage== "I",1,expression_clinical$stage)
#把stage亚型的分类变成数字编号
...
expression_clinical %>% group_by(sex) %>% summarise(sample_number = n(),
                                                    ...
                                                    stage=median(stage),
                                                    smoking = median(as.numeric(smoking))
                                                    )#校正之前先大致看男女样本之间的协变量差异

expression_clinical<-expression_clinical %>% mutate(sex=ifelse(sex=="M",1,0)) 
#把性别变成1,0格式,男性[223]为1，女性[47]为0
mc_ps <- glm(sex ~ age  + stage + smoking ,
...
clinical_cov<-c("age","stage","smoking") #挑选协变量
...
mod_match <- matchit(sex ~ age +stage+smoking ,
...
pheatmap(df,cellwidth =1.5, cellheight = 4, fontsize = 6,fontsize_row=4,

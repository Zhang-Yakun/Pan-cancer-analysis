setwd("D:/workplace/mywork/revise/GEO/GSE41613/")
rm(list=ls())
...
sur.df<-rbind(sample,sex,age,stage,status,time)
colnames(sur.df)<-sur.df[1,]

sur.df2<-t(sur.df)
colnames(sur.df2)<-c("sample","sex","age","stage","status","time")
sur.df2<-as.data.frame(sur.df2)

sur.df2$sex<-sur.df2$sex %>%str_remove("Sex: ")
sur.df2$status<-ifelse(sur.df2$status=="vital: Alive",0,1)%>%as.numeric()
sur.df2$time<-sur.df2$time%>%str_remove("fu time: ")%>%as.numeric()

sur.df2$age<-sur.df2$age%>%str_remove("age: ")
sur.df2$age<-sur.df2$age%>%str_sub(1,2)%>%as.numeric()

sur.df2$stage<-sur.df2$stage%>%str_remove("tumor stage: ")%>%as.factor()
...
#把性别变成1,0格式,男性[66]为1，女性[31]为0
mc_ps <- glm(sex ~ age  + stage ,
...
clinical_cov<-c("age","stage") #挑选协变量
...
mod_match <- matchit(sex ~ age +stage ,
...
dim(dta_m)  #匹配后剩余62个样本

write.csv(dta_m,"GSE41613_PSM.csv")
...
heat_cov<-left_join(heat_sample,sur.df3[,1:4])  #热图中的临床信息
...
pheatmap(df,cellwidth =2, cellheight = 4, fontsize = 6,fontsize_row=4,

setwd("D:/workplace/mywork/revise/GEO/GSE117973/")
rm(list=ls())
...
exp<-fread("GSE117973_series_matrix.txt")%>%as.data.frame()
...
sur.df<-rbind(sample,sex,age,smoking,alcohol,status,time)
colnames(sur.df)<-sur.df[1,]

sur.df2<-t(sur.df)
colnames(sur.df2)<-c("sample","sex","age","smoking","alcohol","status","time")
...
sur.df2$age<-sur.df2$age%>%str_remove("age \\(years\\)\\: ")%>%as.numeric()

sur.df2$smoking<-ifelse(sur.df2$smoking=="smoker: no",0,1)

sur.df2$alcohol<-ifelse(sur.df2$alcohol=="alcohol: no",0,1)
...
mc_ps <- glm(sex ~ age  + smoking +alcohol ,
...
clinical_cov<-c("age","smoking","alcohol") #挑选协变量
...
mod_match <- matchit(sex ~ age +smoking+alcohol ,
...
write.csv(dta_m,"GSE117973_PSM.csv")
...
pheatmap(df,cellwidth =4, cellheight = 4, fontsize = 6,fontsize_row=4,
